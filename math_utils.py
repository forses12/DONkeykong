...
def vibor_ygol(ugol_tochki, ugol_samoleta):
    """
    :param ugol_tochki: под каким углом к самолету находится точка
    :param ugol_samoleta: под каким углом смотрит самолет
    :return: Возвращает правильный угол точки, по которому самолету будет быстрее всего повернуться к точке.
    """
    ugol_samoleta=-ugol_samoleta
    ugol_tochki=-ugol_tochki
    angle_pointm1 = ugol_tochki - 360
    angle_pointm2 = ugol_tochki - 360*2
    angle_pointp1 = ugol_tochki + 360
    angle_pointp2 = ugol_tochki + 360*2
    s = [abs(angle_pointm2 - ugol_samoleta), abs(angle_pointm1 - ugol_samoleta), abs(ugol_tochki - ugol_samoleta),
         abs(angle_pointp1 - ugol_samoleta), abs(angle_pointp2 - ugol_samoleta)]
    s1 = min(s)

    if s1 == s[0]:
        ugol_tochki = angle_pointm2
    elif s1==s[1]:
        ugol_tochki = angle_pointm1
    elif s1 == s[3]:
        ugol_tochki = angle_pointp1
    elif s1 == s[4]:
